...
export class BackgroundPontoonClient {
  ...
  private readonly notificationsChangeListener: (message: { type: string, data: unknown }, sender: Runtime.MessageSender) => void;

  constructor() {
    this.notificationsChangeCallbacks = new Set();
    this.notificationsChangeListener = (message, _sender) => this.backgroundMessage(message);
  }
  ...
  subscribeToNotificationsChange(callback: NotificationsChangeCallback): void {
    ...
    if (!browser.runtime.onMessage.hasListener(this.notificationsChangeListener)) {
  ...
  unsubscribeFromNotificationsChange(callback: NotificationsChangeCallback): void {
    ...
    if (deleted && this.notificationsChangeCallbacks.size === 0) {
      browser.runtime.onMessage.removeListener(this.notificationsChangeListener);
    }
  }
  ...
  private backgroundMessage(message: { type: string, data: unknown }): void {
    if (message.type === BackgroundPontoonMessageType.FROM_BACKGROUND.NOTIFICATIONS_UPDATED) {
      this.notificationsChangeCallbacks.forEach((callback) => callback(message.data as NotificationsChange));
    }
  }
}
