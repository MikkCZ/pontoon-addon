...
export class Options {
  ...
  private static async loadDefaultValues(prefix: string): Promise<Record<string, OptionValue>> {
    ...
    await Promise.all([
      ...
      Object.entries(loadedDefaults)
        .forEach(([key, value]) => defaults[`${prefix}${key}`] = value as OptionValue);
    });
  ...
  private static isValidInput(input: HTMLInputElement | HTMLSelectElement | HTMLElement): boolean {
    return (input.nodeName.toLowerCase() === 'select') || (input instanceof HTMLInputElement && input.type === 'radio') || (input.parentElement?.querySelector(':valid') === input);
  }

  private static getValueFromInput(input: HTMLInputElement | HTMLSelectElement): OptionValue {
  ...
  private static setValueToInputs(inputs: (HTMLInputElement | HTMLSelectElement)[], value: OptionValue): void {
    if (inputs.length === 1) {
  ...
  private loadAllFromObject(object: Record<string, OptionValue>): void {
    Object.entries(object)
      ...
      .forEach(([key, value]) => {
        const inputs = Array.from(document.querySelectorAll(`[data-option-id=${this.getInputId(key)}]`)) as (HTMLInputElement | HTMLSelectElement)[];
        if (inputs.length > 0) {
  ...
  async loadAllFromLocalStorage(): Promise<void> {
    ...
    await browser.storage.local.get().then(
      (items) => this.loadAllFromObject(items)
    );
  }
  ...
  async get(optionIds: OptionId | OptionId[]): Promise<OptionsValues> {
    ...
    return await browser.storage.local.get(optionIdsArray.map((optionId) => `${this.prefix}${optionId}`)).then(
      (items) => {
        ...
        optionIdsArray.forEach((optionId) => {
          ...
          if (items[realOptionId] !== undefined) {
            ...
            optionsWithDefaultValues[optionId] = this.defaultValues[realOptionId];
          }
        });
  ...
  subscribeToOptionChange(optionId: string, callback: (change: Storage.StorageChange) => void): void {
